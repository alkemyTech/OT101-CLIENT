...
const TestimonialForm = ({classes, open, testimonial, onCancel, onSuccess, onFailure}) => {
  ...
  const testimonialSubmit = (values, actions) => {
    ...
    const apiRequest = testimonial && testimonial.id ?
      patchRequest(`http://localhost:3001/testimonials/${testimonial.id}`, dataToSend, {headers: {'content-type': 'multipart/form-data'}}) :
      postRequest('http://localhost:3001/testimonials/', dataToSend, {headers: {'content-type': 'multipart/form-data'}});
    ...
    <Container maxWidth="sm">
      <Paper elevation={3} className={classes.innerBox}>
        <Typography variant="h5" component="h2" fontWeight="bold" mb={6}>
          {testimonial && testimonial.id ? "Modificar" : "Crear" } Actividades
        </Typography>
