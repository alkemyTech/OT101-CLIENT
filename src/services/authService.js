...
export async function login(data) {
	const { firstName, lastName, email, token } = await httpService.postRequest(
		`${process.env.REACT_APP_URL_SERVER}/auth/login`, data
	)
...
export async function register(data) {
	const response = await httpService.postRequest(`${process.env.REACT_APP_URL_SERVER}/auth/register`, data)
...
export async function editProfile(id, data) {
  return httpService.patchRequest(`${process.env.REACT_APP_URL_SERVER}/users/${id}`, data);

}

export async function deleteAccount(id) {
  return httpService.deleteRequest(`${process.env.REACT_APP_URL_SERVER}/users/${id}`);
}